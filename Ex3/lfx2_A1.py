...
def B(W, A, gamma):
    ...
    B = np.zeros_like(W)
    # Divide the conserved variables by the area to get the correct conserved variables
    W[:,0] /= A[:]
    W[:,1] /= A[:]
    W[:,2] /= A[:]
    ...
    for i in range(len(B)-1):
        p_mean = 0.5 * (p[i+1] + p[i])
        B[i,1] = - p_mean * (A[i+1] - A[i])

    # The last cell does not have a right neighbor, so we set it to the same value
    # as the last cell
    B[-1,1] = B[-1,1]
...
def lfx2_step(U_grid, A_grid, A_int, dx, c, gamma = 1.4):
    ...
    A_grid_gc = np.concatenate(([A_grid[0]], A_grid, [A_grid[-1]]))
    
    W_grid_gc = U2W(U_grid_gc, gamma = gamma)
    a_grid_gc = np.zeros((n_samples + 2, 1))

    # Multiply the conserved variables by the area to get the correct conserved variables
    W_grid_gc[:,0] *= A_grid_gc[:]
    W_grid_gc[:,1] *= A_grid_gc[:]
    W_grid_gc[:,2] *= A_grid_gc[:]
    ...
    # Compute the fluxes
    F_grid_gc = Flux(W_grid_gc, A_grid_gc, gamma = gamma)
    # Compute the source term
    B_grid = B(U2W(U_grid, gamma=gamma), A_grid, gamma = gamma)
    B_grid_gc = np.concatenate((B_grid[0].reshape(1, -1), B_grid, B_grid[-1].reshape(1, -1)), axis = 0)
    ...
    W_half_p = 0.5 * (W_grid_gc[2:] + W_grid_gc[1:-1]) 
    - 0.5 * dt / dx * (F_grid_gc[2:] - F_grid_gc[1:-1]) 
    
    W_half_m = 0.5 * (W_grid_gc[1:-1] + W_grid_gc[:-2])
    - 0.5 * dt / dx * (F_grid_gc[1:-1] - F_grid_gc[:-2])
    
    # Add the source term to the half-steps
    W_half_p += 0.5*dt / dx * B_grid_gc[1:-1]
    W_half_m += 0.5*dt / dx * B_grid_gc[0:-2]

    # Compute the fluxes at the half-steps
    F_half_p = Flux(W_half_p, A_grid, gamma = gamma)
    F_half_m = Flux(W_half_m, A_grid, gamma = gamma)

    # Compute the source term at the temporal half-step
    B_n_half = np.zeros_like(U_grid)
    p_p = (gamma - 1) * (W_half_p[:,2] - 0.5 * W_half_p[:,1] ** 2 / W_half_p[:,0])
    p_m = (gamma - 1) * (W_half_m[:,2] - 0.5 * W_half_m[:,1] ** 2 / W_half_m[:,0])

    B_n_half[:, 1] = (p_p[1:] + p_m[:-1]) / 2 * (A_int[1:] - A_int[:-1])

    
    # Correction step
    W_grid_dt = np.zeros_like(U_grid)
    W_grid_dt = (W_half_m + W_half_p)/2 - 0.5 *dt / dx * (F_half_p - F_half_m) + 0.5 * dt / dx * B_n_half[1:-1]
    
    # Divide by the area to get the correct conserved variables
    W_grid_dt[:,0] /= A_grid[:]
    W_grid_dt[:,1] /= A_grid[:]
    W_grid_dt[:,2] /= A_grid[:]
...
def LFx2(U0, A, A_interface, dx, c, T, gamma = 1.4):
    ...
    # Simulation loop
    while (t_total < T):
        U_grid, a_grid, dt = lfx2_step(U_grid, A, A_interface, dx, c, gamma = gamma)
        t_total += dt
